...
Push the packages-feed compatibility branch, then dispatch both release workflows with `firmware_release=true`. Read each run once to confirm the new commit SHA and initial status; do not wait for completion.
